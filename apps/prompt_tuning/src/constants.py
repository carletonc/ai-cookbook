...
While this application demonstrates a proof-of-concept, it is intended to be adapted for specific use cases. This toy example tunes a prompt for analysis on a single dataset, but it could be modified to tune a prompt across multiple sets of inputs and outputs, observing the evaluations at scale, and iteratively improving the input prompt. To manage this complexity, consider expanding the `Optimizer Prompt` to two steps. You could use one LLM to analyze the errors across a dataset at scale and attribute labels to them, much like error analysis, and then use an additional LLM to observe the error analysis and suggest small improvements to the input prompt as we iterate (think of this as a neural net's learning rate). This framework is even extensible to early stopping criteria and evaluation on holdout sets.
